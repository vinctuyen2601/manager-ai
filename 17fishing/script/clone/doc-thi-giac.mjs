...
print(json.dumps({'soAnh': len(sys.argv) - 1, 'mauDinhDanh': chinh, 'mauNen': nen},
                 ensure_ascii=False))
`;
...
let kq;
try {
  kq = JSON.parse(execSync(`python3 ${JSON.stringify(tepPy)} ${tep.map((t) => JSON.stringify(t)).join(' ')}`).toString());
} finally {
  fs.unlinkSync(tepPy);
}
